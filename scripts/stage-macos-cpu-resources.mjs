...
function main() {
  ...
  for (const artifact of manifest.artifacts) {
    const staged = stageArtifact(artifact, resourceRoot);
...
function stageArtifact(artifact, resourceRoot) {
  ...
  const digest = sha256Directory(destination);
  return {
    id: artifact.id,
    kind: artifact.kind,
...
function stagePythonRuntime(venvSource, destination) {
  ...
  const baseSource =
    process.env.CLEARPODCAST_MACOS_PYTHON_BASE_SOURCE ??
    pyvenv.home?.replace(/\/bin$/, "");

  if (!baseSource || !fs.existsSync(baseSource)) {
    throw new Error(
      `could not resolve a self-contained Python base for ${venvSource}; set CLEARPODCAST_MACOS_PYTHON_BASE_SOURCE`,
    );
  }
  ...
  const pythonVersion = pyvenv.version?.match(/^\d+\.\d+/)?.[0] ?? "3.12";
  ...
  for (const pythonName of ["python", "python3", `python${pythonVersion}`]) {
    const executable = path.join(destination, "bin", pythonName);
    if (!fs.existsSync(executable)) {
      throw new Error(`staged Python executable was not found: ${executable}`);
    }
  }

  repairAbsoluteSymlinks(destination);
  const leakedSymlinks = findAbsoluteSymlinks(destination);
  if (leakedSymlinks.length > 0) {
    throw new Error(
      `staged runtime contains absolute symlinks:\n${leakedSymlinks.join("\n")}`,
    );
  }
}
...
function findAbsoluteSymlinks(directory) {
  return collectEntries(directory)
    .filter((entry) => entry.kind === "symlink")
    .map((entry) => [entry, fs.readlinkSync(entry.absolute)])
    .filter(([, target]) => path.isAbsolute(target))
    .map(([entry, target]) => `${entry.relative} -> ${target}`);
}

function repairAbsoluteSymlinks(directory) {
  for (const entry of collectEntries(directory).filter(
    (candidate) => candidate.kind === "symlink",
  )) {
    ...
    const localTarget = path.join(path.dirname(entry.absolute), path.basename(target));
    if (!fs.existsSync(localTarget)) {
      continue;
    }

    fs.unlinkSync(entry.absolute);
    fs.symlinkSync(path.basename(target), entry.absolute);
  }
}
...
function overrideSource(artifact) {
  if (artifact.kind === "runtime") {
    return process.env.CLEARPODCAST_MACOS_RUNTIME_SOURCE;
  }
